...
from v3_async_wip import alt_sastoken as st
...
class IoTHubSession:
    ...
    async def __aenter__(self) -> "IoTHubSession":
        ...
        if self._sas_auth:
            ...
            await self._sas_auth.start()

        # Now connect
        try:
            await self.connect()
        except (Exception, asyncio.CancelledError):
            await self.__aexit__(None, None, None)  # TODO: how to do this correctly

        return self

    async def __aexit__(self, exc_type, exc, tb) -> None:
        if self._sas_auth:
            await self._sas_auth.stop()
