...
from azure.iot.hub.devicesdk.sync_clients import ModuleClientSync, DeviceClientSync, GenericClientSync
...
def test_generic_client_sync_connect_in_turn_calls_transport_connect(
    authentication_provider, mock_transport
):
    client = GenericClientSync(authentication_provider, mock_transport)
...
def test_generic_client_sync_send_event_in_turn_calls_transport_send_event(
    authentication_provider, mock_transport
):
...
def test_transport_any_error_surfaces_to_generic_client_sync(authentication_provider, mock_transport):
...
def test_client_gets_created_correctly(mocker, kind_of_client, auth, mock_transport):
    mock_constructor_transport = mocker.patch(
        "azure.iot.hub.devicesdk.sync_clients.MQTTTransport"
    )
    mock_constructor_transport.return_value = mock_transport

    if kind_of_client == "Module":
        module_client = ModuleClientSync.from_authentication_provider(authentication_provider, "mqtt")

        assert module_client._auth_provider == authentication_provider
        assert module_client._transport == mock_transport
        assert isinstance(module_client, ModuleClientSync)

    else:
        device_client = DeviceClientSync.from_authentication_provider(authentication_provider, "mqtt")

        assert device_client._auth_provider == authentication_provider
        assert device_client._transport == mock_transport
        assert isinstance(device_client, DeviceClientSync)
...
def test_raises_on_creation_of_client_when_transport_is_incorrect(kind_of_client, auth):
    with pytest.raises(
        ...
        if kind_of_client == "Module":
            ModuleClientSync.from_authentication_provider(authentication_provider, "floo")
        else:
            DeviceClientSync.from_authentication_provider(authentication_provider, "floo")
