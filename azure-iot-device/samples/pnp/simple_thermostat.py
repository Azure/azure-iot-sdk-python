...
from azure.iot.device import constant, Properties, CommandResponse, WritablePropertyResponse
...
async def handle_writable_property_patch_received(device_client, writable_props):
    ...
    for prop_name in writable_props.property_values:
        patch.property_values[prop_name] = WritablePropertyResponse(
            ac=200,
...
async def main():
    ...
    await device_client.send_propertyPatch(
        Properties.from_dict({"maxTempSinceLastReboot": max_temp})
    )
