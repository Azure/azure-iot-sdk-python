...
def add_base_pipeline_stage_tests(
    ...
    # @pytest.mark.describe("{} - Method Threading".format(stage_class_under_test.__name__))
    # class StageThreadingTests(StageTestConfig):
    #     pass

    # # List of names of methods which we will test the threading rules for.
    # # Exclude magic/dunder methods (i.e. methods that begin with "__"), as they do not have thread rules
    # methods_to_test = [method_data[0] for method_data in inspect.getmembers(stage_class_under_test) if not method_data[0].startswith("__") and not method_data[0] == "_abc_impl"]

    # for method_name in methods_to_test:

    #     @pytest.mark.it("Prevents invocations of {} from outside of the pipeline thread".format(method_name))
    #     def test_method_threading(self, mocker, stage):
    #         method = getattr(stage, method_name)
    #         args = [mocker.MagicMock() for i in (range(get_arg_count(method) - 1))]
    #         with pytest.raises(AssertionError):
    #             method(*args)

    #     setattr(StageThreadingTests, "test_{}_threading".format(method_name), test_method_threading)

    # setattr(test_module, "Test{}Threading".format(stage_class_under_test.__name__), StageThreadingTests)
