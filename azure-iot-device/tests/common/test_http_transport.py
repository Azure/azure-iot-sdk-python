...
class TestRequest(object):
    ...
    async def test_creates_http_connection_object(self, mocker):
        ...
        def request_callback(response=None, error=None):
            ...
            assert mock_http_connection_constructor.call_count == 1

        await transport.request(fake_method, fake_path, request_callback)
        # assert mock_http_client.call_count == 1
        # transport.request(fake_method, fake_path, request_callback2)


# @pytest.mark.describe("HTTPTransport - .connect()")
# class TestConnect(object):
#     @pytest.mark.it("Uses the stored username and provided password for Paho credentials")
#     def test_use_provided_password(self, mocker, mock_http_client, transport):
#         transport.connect(fake_password)

#         assert mock_http_client.username_pw_set.call_count == 1
#         assert mock_http_client.username_pw_set.call_args == mocker.call(
...
#     )
#     def test_use_no_password(self, mocker, mock_http_client, transport):
#         transport.connect()
...
#         self, mocker, mock_http_client, transport, password, websockets, port
#     ):
...
#         assert mock_http_client.connect.call_count == 1
#         assert mock_http_client.connect.call_args == mocker.call(
...
#         ],
#     )
#     def test_calls_paho_connect_with_keepalive(self, mocker, mock_http_client, transport, password):

#         http_transport.DEFAULT_KEEPALIVE = fake_keepalive
...
#         ],
#     )
#     def test_calls_loop_start(self, mocker, mock_http_client, transport, password):
#         transport.connect(password)

#         assert mock_http_client.loop_start.call_count == 1
#         assert mock_http_client.loop_start.call_args == mocker.call()
...
#         self, mocker, mock_http_client, transport, arbitrary_exception
#     ):
#         mock_http_client.connect.side_effect = arbitrary_exception
...
#         mock_http_client.connect.side_effect = socket_error
...
#         self, mock_http_client, transport, arbitrary_base_exception
#     ):
#         mock_http_client.connect.side_effect = arbitrary_base_exception
...
#         self, mocker, mock_http_client, transport, error_params
#     ):
#         mock_http_client.connect.return_value = error_params["rc"]
#         with pytest.raises(error_params["error"]):
#             transport.connect(fake_password)


# @pytest.mark.describe("HTTPTransport - .reauthorize_connection()")
...
#         ],
#     )
#     def test_calls_paho_reconnect(self, mocker, mock_http_client, transport, password):
#         transport.reauthorize_connection(password)

#         assert mock_http_client.reconnect.call_count == 1
#         assert mock_http_client.reconnect.call_args == mocker.call()

#     @pytest.mark.it("Raises a ProtocolClientError if Paho reconnect raises an unexpected Exception")
#     def test_client_raises_unexpected_error(
#         self, mocker, mock_http_client, transport, arbitrary_exception
#     ):
#         mock_http_client.reconnect.side_effect = arbitrary_exception
#         with pytest.raises(errors.ProtocolClientError) as e_info:
#             transport.reauthorize_connection(fake_password)
#         assert e_info.value.__cause__ is arbitrary_exception
...
#         self, mock_http_client, transport, arbitrary_base_exception
#     ):
#         mock_http_client.reconnect.side_effect = arbitrary_base_exception
#         with pytest.raises(arbitrary_base_exception.__class__) as e_info:
#             transport.reauthorize_connection(fake_password)
#         assert e_info.value is arbitrary_base_exception
...
#         self, mocker, mock_http_client, transport, error_params
#     ):
#         mock_http_client.reconnect.return_value = error_params["rc"]
#         with pytest.raises(error_params["error"]):
#             transport.reauthorize_connection(fake_password)


# @pytest.mark.describe("HTTPTransport - EVENT: Connect Completed")
# class TestEventConnectComplete(object):
#     @pytest.mark.it(
#         "Triggers on_http_connected_handler event handler upon successful connect completion"
#     )
#     def test_calls_event_handler_callback(self, mocker, mock_http_client, transport):
#         callback = mocker.MagicMock()
#         transport.on_http_connected_handler = callback

#         # Manually trigger Paho on_connect event_handler
#         mock_http_client.on_connect(client=mock_http_client, userdata=None, flags=None, rc=fake_rc)

#         # Verify transport.on_http_connected_handler was called
#         assert callback.call_count == 1
#         assert callback.call_args == mocker.call()

#     @pytest.mark.it(
#         "Skips on_http_connected_handler event handler if set to 'None' upon successful connect completion"
#     )
#     def test_skips_none_event_handler_callback(self, mocker, mock_http_client, transport):
#         assert transport.on_http_connected_handler is None
...
#     @pytest.mark.it("Recovers from Exception in on_http_connected_handler event handler")
#     def test_event_handler_callback_raises_exception(
#         self, mocker, mock_http_client, transport, arbitrary_exception
#     ):
#         event_cb = mocker.MagicMock(side_effect=arbitrary_exception)
#         transport.on_http_connected_handler = event_cb
...
#     @pytest.mark.it(
#         "Allows any BaseExceptions raised in on_http_connected_handler event handler to propagate"
#     )
#     def test_event_handler_callback_raises_base_exception(
#         self, mocker, mock_http_client, transport, arbitrary_base_exception
#     ):
...
#         transport.connect(fake_password)
#         with pytest.raises(arbitrary_base_exception.__class__) as e_info:
#             mock_http_client.on_connect(
#                 client=mock_http_client, userdata=None, flags=None, rc=fake_rc
#             )
#         assert e_info.value is arbitrary_base_exception


# @pytest.mark.describe("HTTPTransport - EVENT: Connection Failure")
...
#     )
#     @pytest.mark.it(
#         "Triggers on_http_connection_failure_handler event handler with custom Exception upon failed connect completion"
#     )
#     def test_calls_event_handler_callback_with_failed_rc(
#         self, mocker, mock_http_client, transport, error_params
#     ):
#         callback = mocker.MagicMock()
#         transport.on_http_connection_failure_handler = callback
...
#         # Manually trigger Paho on_connect event_handler
#         mock_http_client.on_connect(
#             client=mock_http_client, userdata=None, flags=None, rc=error_params["rc"]
#         )

#         # Verify transport.on_http_connection_failure_handler was called
#         assert callback.call_count == 1
#         assert isinstance(callback.call_args[0][0], error_params["error"])

#     @pytest.mark.it(
#         "Skips on_http_connection_failure_handler event handler if set to 'None' upon failed connect completion"
#     )
#     def test_skips_none_event_handler_callback(self, mocker, mock_http_client, transport):
#         assert transport.on_http_connection_failure_handler is None

#         transport.connect(fake_password)

#         mock_http_client.on_connect(
#             client=mock_http_client, userdata=None, flags=None, rc=failed_connack_rc
#         )

#         # No further asserts required - this is a test to show that it skips a callback.
#         # Not raising an exception == test passed

#     @pytest.mark.it("Recovers from Exception in on_http_connection_failure_handler event handler")
#     def test_event_handler_callback_raises_exception(
#         self, mocker, mock_http_client, transport, arbitrary_exception
#     ):
#         event_cb = mocker.MagicMock(side_effect=arbitrary_exception)
#         transport.on_http_connection_failure_handler = event_cb
...
#     @pytest.mark.it(
#         "Allows any BaseExceptions raised in on_http_connection_failure_handler event handler to propagate"
#     )
#     def test_event_handler_callback_raises_base_exception(
#         self, mocker, mock_http_client, transport, arbitrary_base_exception
#     ):
...
# @pytest.mark.describe("HTTPTransport - .disconnect()")
# class TestDisconnect(object):
#     @pytest.mark.it("Initiates MQTT disconnect via Paho")
#     def test_calls_paho_disconnect(self, mocker, mock_http_client, transport):
#         transport.disconnect()

#         assert mock_http_client.disconnect.call_count == 1
#         assert mock_http_client.disconnect.call_args == mocker.call()

#     @pytest.mark.it("Stops MQTT Network Loop")
#     def test_calls_loop_stop(self, mocker, mock_http_client, transport):
#         transport.disconnect()

#         assert mock_http_client.loop_stop.call_count == 1
#         assert mock_http_client.loop_stop.call_args == mocker.call()
...
#         self, mocker, mock_http_client, transport, arbitrary_exception
#     ):
#         mock_http_client.disconnect.side_effect = arbitrary_exception
#         with pytest.raises(errors.ProtocolClientError) as e_info:
#             transport.disconnect()
#         assert e_info.value.__cause__ is arbitrary_exception
...
#         self, mock_http_client, transport, arbitrary_base_exception
#     ):
#         mock_http_client.disconnect.side_effect = arbitrary_base_exception
#         with pytest.raises(arbitrary_base_exception.__class__) as e_info:
#             transport.disconnect()
#         assert e_info.value is arbitrary_base_exception
...
#         self, mocker, mock_http_client, transport, error_params
#     ):
#         mock_http_client.disconnect.return_value = error_params["rc"]
#         with pytest.raises(error_params["error"]):
#             transport.disconnect()
...
#         ],
#     )
#     def test_client_drops_connection(self, mock_http_client, transport, error_params):
#         mock_http_client.disconnect.return_value = error_params["rc"]
#         transport.disconnect()
...
# @pytest.mark.describe("HTTPTransport - EVENT: Disconnect Completed")
# class TestEventDisconnectCompleted(object):
#     @pytest.mark.it(
#         "Triggers on_http_disconnected_handler event handler upon disconnect completion"
#     )
#     def test_calls_event_handler_callback_externally_driven(
#         self, mocker, mock_http_client, transport
#     ):
#         callback = mocker.MagicMock()
#         transport.on_http_disconnected_handler = callback

#         # Initiate disconnect
#         transport.disconnect()

#         # Manually trigger Paho on_connect event_handler
#         mock_http_client.on_disconnect(client=mock_http_client, userdata=None, rc=fake_rc)
...
#     )
#     @pytest.mark.it(
#         "Triggers on_http_disconnected_handler event handler with custom Exception when an error RC is returned upon disconnect competion."
#     )
...
#         mock_http_client.on_disconnect(
#             client=mock_http_client, userdata=None, rc=error_params["rc"]
#         )

#         # Verify transport.on_http_disconnected_handler was called
#         assert callback.call_count == 1
#         assert isinstance(callback.call_args[0][0], error_params["error"])

#     @pytest.mark.it(
#         "Skips on_http_disconnected_handler event handler if set to 'None' upon disconnect completion"
#     )
#     def test_skips_none_event_handler_callback(self, mocker, mock_http_client, transport):
#         assert transport.on_http_disconnected_handler is None
...
#     @pytest.mark.it("Recovers from Exception in on_http_disconnected_handler event handler")
#     def test_event_handler_callback_raises_exception(
#         self, mocker, mock_http_client, transport, arbitrary_exception
#     ):
#         event_cb = mocker.MagicMock(side_effect=arbitrary_exception)
#         transport.on_http_disconnected_handler = event_cb
...
#     @pytest.mark.it(
#         "Allows any BaseExceptions raised in on_http_disconnected_handler event handler to propagate"
#     )
#     def test_event_handler_callback_raises_base_exception(
#         self, mocker, mock_http_client, transport, arbitrary_base_exception
#     ):
...
#     def test_calls_disconnect_with_cause(self, mock_http_client, transport):
#         mock_http_client.on_disconnect(client=mock_http_client, userdata=None, rc=fake_failed_rc)
#         assert mock_http_client.disconnect.call_count == 1

#     @pytest.mark.it("Does not call Paho's disconnect() method if cause is None")
#     def test_doesnt_call_disconnect_without_cause(self, mock_http_client, transport):
#         mock_http_client.on_disconnect(client=mock_http_client, userdata=None, rc=fake_success_rc)
#         assert mock_http_client.disconnect.call_count == 0

#     @pytest.mark.it("Calls Paho's loop_stop() if cause is not None")
#     def test_calls_loop_stop(self, mock_http_client, transport):
#         mock_http_client.on_disconnect(client=mock_http_client, userdata=None, rc=fake_failed_rc)
#         assert mock_http_client.loop_stop.call_count == 1

#     @pytest.mark.it("Does not calls Paho's loop_stop() if cause is None")
#     def test_does_not_call_loop_stop(self, mock_http_client, transport):
#         mock_http_client.on_disconnect(client=mock_http_client, userdata=None, rc=fake_success_rc)
#         assert mock_http_client.loop_stop.call_count == 0

#     @pytest.mark.it("Sets Paho's _thread to None if cause is not None")
#     def test_sets_thread_to_none(self, mock_http_client, transport):
#         mock_http_client._thread = fake_thread
#         mock_http_client.on_disconnect(client=mock_http_client, userdata=None, rc=fake_failed_rc)
#         assert mock_http_client._thread is None

#     @pytest.mark.it("Does not sets Paho's _thread to None if cause is None")
#     def test_does_not_set_thread_to_none(self, mock_http_client, transport):
#         mock_http_client._thread = fake_thread
#         mock_http_client.on_disconnect(client=mock_http_client, userdata=None, rc=fake_success_rc)
#         assert mock_http_client._thread == fake_thread

#     @pytest.mark.it("Allows any Exception raised by Paho's disconnect() to propagate")
#     def test_disconnect_raises_exception(
#         self, mock_http_client, transport, mocker, arbitrary_exception
#     ):
#         mock_http_client.disconnect = mocker.MagicMock(side_effect=arbitrary_exception)
#         with pytest.raises(type(arbitrary_exception)) as e_info:
#             mock_http_client.on_disconnect(
#                 client=mock_http_client, userdata=None, rc=fake_failed_rc
#             )
...
#         self, mock_http_client, transport, mocker, arbitrary_base_exception
#     ):
#         mock_http_client.disconnect = mocker.MagicMock(side_effect=arbitrary_base_exception)
...
#         self, mock_http_client, transport, mocker, arbitrary_exception
#     ):
#         mock_http_client.loop_stop = mocker.MagicMock(side_effect=arbitrary_exception)
#         with pytest.raises(type(arbitrary_exception)) as e_info:
#             mock_http_client.on_disconnect(
#                 client=mock_http_client, userdata=None, rc=fake_failed_rc
#             )
...
#         self, mock_http_client, transport, mocker, arbitrary_base_exception
#     ):
#         mock_http_client.loop_stop = mocker.MagicMock(side_effect=arbitrary_base_exception)
#         with pytest.raises(type(arbitrary_base_exception)) as e_info:
#             mock_http_client.on_disconnect(
#                 client=mock_http_client, userdata=None, rc=fake_failed_rc
#             )
...
# @pytest.mark.describe("HTTPTransport - .subscribe()")
...
#     )
#     def test_calls_paho_subscribe(self, mocker, mock_http_client, transport, qos):
#         transport.subscribe(fake_topic, qos=qos)

#         assert mock_http_client.subscribe.call_count == 1
#         assert mock_http_client.subscribe.call_args == mocker.call(fake_topic, qos=qos)
...
#     ):
#         callback = mocker.MagicMock()
#         mock_http_client.subscribe.return_value = (fake_rc, fake_mid)
...
#         mock_http_client.on_subscribe(
#             client=mock_http_client, userdata=None, mid=fake_mid, granted_qos=fake_qos
#         )
...
#         mock_http_client.subscribe.side_effect = trigger_early_on_subscribe
...
#     def test_none_callback_upon_paho_on_subscribe_event(self, mocker, mock_http_client, transport):
#         callback = None
#         mock_http_client.subscribe.return_value = (fake_rc, fake_mid)
...
#     )
#     def test_multiple_callbacks(self, mocker, mock_http_client, transport):
...
#         mock_http_client.subscribe.side_effect = [(fake_rc, mid1), (fake_rc, mid2), (fake_rc, mid3)]
...
#         mock_http_client.on_subscribe(
#             client=mock_http_client, userdata=None, mid=mid2, granted_qos=fake_qos
#         )
#         assert callback1.call_count == 0
...
#         mock_http_client.on_subscribe(
#             client=mock_http_client, userdata=None, mid=mid3, granted_qos=fake_qos
#         )
#         assert callback1.call_count == 0
#         assert callback2.call_count == 1
...
#         mock_http_client.on_subscribe(
#             client=mock_http_client, userdata=None, mid=mid1, granted_qos=fake_qos
#         )
...
#         self, mocker, mock_http_client, transport, arbitrary_exception
#     ):
#         mock_http_client.subscribe.side_effect = arbitrary_exception
#         with pytest.raises(errors.ProtocolClientError) as e_info:
...
#         self, mock_http_client, transport, arbitrary_base_exception
#     ):
#         mock_http_client.subscribe.side_effect = arbitrary_base_exception
#         with pytest.raises(arbitrary_base_exception.__class__) as e_info:
#             transport.subscribe(topic=fake_topic, qos=fake_qos, callback=None)
#         assert e_info.value is arbitrary_base_exception
...
#         self, mocker, mock_http_client, transport, error_params
#     ):
#         mock_http_client.subscribe.return_value = (error_params["rc"], 0)
#         with pytest.raises(error_params["error"]):
#             transport.subscribe(topic=fake_topic, qos=fake_qos, callback=None)


# @pytest.mark.describe("HTTPTransport - .unsubscribe()")
# class TestUnsubscribe(object):
#     @pytest.mark.it("Unsubscribes with Paho")
#     def test_calls_paho_unsubscribe(self, mocker, mock_http_client, transport):
#         transport.unsubscribe(fake_topic)

#         assert mock_http_client.unsubscribe.call_count == 1
#         assert mock_http_client.unsubscribe.call_args == mocker.call(fake_topic)
...
#     ):
#         callback = mocker.MagicMock()
#         mock_http_client.unsubscribe.return_value = (fake_rc, fake_mid)
...
#         mock_http_client.on_unsubscribe(client=mock_http_client, userdata=None, mid=fake_mid)
...
#         mock_http_client.unsubscribe.side_effect = trigger_early_on_unsubscribe
...
#         mock_http_client.unsubscribe.side_effect = [
#             (fake_rc, mid1),
#             (fake_rc, mid2),
#             (fake_rc, mid3),
#         ]
...
#         mock_http_client.on_unsubscribe(client=mock_http_client, userdata=None, mid=mid2)
#         assert callback1.call_count == 0
#         assert callback2.call_count == 1
#         assert callback3.call_count == 0

#         mock_http_client.on_unsubscribe(client=mock_http_client, userdata=None, mid=mid3)
#         assert callback1.call_count == 0
#         assert callback2.call_count == 1
#         assert callback3.call_count == 1

#         mock_http_client.on_unsubscribe(client=mock_http_client, userdata=None, mid=mid1)
#         assert callback1.call_count == 1
#         assert callback2.call_count == 1
#         assert callback3.call_count == 1
...
#         self, mocker, mock_http_client, transport, arbitrary_exception
#     ):
#         mock_http_client.unsubscribe.side_effect = arbitrary_exception
#         with pytest.raises(errors.ProtocolClientError) as e_info:
...
#         self, mock_http_client, transport, arbitrary_base_exception
#     ):
#         mock_http_client.unsubscribe.side_effect = arbitrary_base_exception
#         with pytest.raises(arbitrary_base_exception.__class__) as e_info:
#             transport.unsubscribe(topic=fake_topic, callback=None)
#         assert e_info.value is arbitrary_base_exception
...
#         self, mocker, mock_http_client, transport, error_params
#     ):
#         mock_http_client.unsubscribe.return_value = (error_params["rc"], 0)
#         with pytest.raises(error_params["error"]):
#             transport.unsubscribe(topic=fake_topic, callback=None)


# @pytest.mark.describe("HTTPTransport - .publish()")
# class TestPublish(object):
#     @pytest.fixture
#     def message_info(self, mocker):
#         mi = http.MQTTMessageInfo(fake_mid)
#         mi.rc = fake_rc
#         return mi
...
#         [pytest.param(0, id="QoS 0"), pytest.param(1, id="QoS 1"), pytest.param(2, id="QoS 2")],
#     )
#     def test_calls_paho_publish(self, mocker, mock_http_client, transport, qos):
#         transport.publish(topic=fake_topic, payload=fake_payload, qos=qos)

#         assert mock_http_client.publish.call_count == 1
#         assert mock_http_client.publish.call_args == mocker.call(
...
#         self, mocker, mock_http_client, transport, message_info
#     ):
#         callback = mocker.MagicMock()
#         mock_http_client.publish.return_value = message_info
...
#         mock_http_client.on_publish(client=mock_http_client, userdata=None, mid=message_info.mid)
...
#             mock_http_client.on_publish(
#                 client=mock_http_client, userdata=None, mid=message_info.mid
#             )
...
#         mock_http_client.publish.side_effect = trigger_early_on_publish
...
#         mock_http_client.publish.side_effect = [
#             http.MQTTMessageInfo(mid1),
#             http.MQTTMessageInfo(mid2),
#             http.MQTTMessageInfo(mid3),
#         ]
...
#         mock_http_client.on_publish(client=mock_http_client, userdata=None, mid=mid2)
#         assert callback1.call_count == 0
#         assert callback2.call_count == 1
#         assert callback3.call_count == 0

#         mock_http_client.on_publish(client=mock_http_client, userdata=None, mid=mid3)
#         assert callback1.call_count == 0
#         assert callback2.call_count == 1
#         assert callback3.call_count == 1

#         mock_http_client.on_publish(client=mock_http_client, userdata=None, mid=mid1)
#         assert callback1.call_count == 1
#         assert callback2.call_count == 1
#         assert callback3.call_count == 1
...
#     @pytest.mark.it("Recovers from Exception in callback")
#     def test_callback_raises_exception(
#         self, mocker, mock_http_client, transport, message_info, arbitrary_exception
#     ):
#         callback = mocker.MagicMock(side_effect=arbitrary_exception)
#         mock_http_client.publish.return_value = message_info
...
#     @pytest.mark.it("Allows any BaseExceptions raised in callback to propagate")
#     def test_callback_raises_base_exception(
#         self, mocker, mock_http_client, transport, message_info, arbitrary_base_exception
#     ):
#         callback = mocker.MagicMock(side_effect=arbitrary_base_exception)
#         mock_http_client.publish.return_value = message_info
...
#         self, mocker, mock_http_client, transport, arbitrary_exception
#     ):
#         mock_http_client.publish.side_effect = arbitrary_exception
#         with pytest.raises(errors.ProtocolClientError) as e_info:
...
#         self, mock_http_client, transport, arbitrary_base_exception
#     ):
#         mock_http_client.publish.side_effect = arbitrary_base_exception
#         with pytest.raises(arbitrary_base_exception.__class__) as e_info:
#             transport.publish(topic=fake_topic, payload=fake_payload, callback=None)
#         assert e_info.value is arbitrary_base_exception
...
#         self, mocker, mock_http_client, transport, error_params
#     ):
#         mock_http_client.publish.return_value = (error_params["rc"], 0)
#         with pytest.raises(error_params["error"]):
#             transport.publish(topic=fake_topic, payload=fake_payload, callback=None)


# @pytest.mark.describe("HTTPTransport - EVENT: Message Received")
# class TestMessageReceived(object):
#     @pytest.fixture()
#     def message(self):
#         message = http.MQTTMessage(mid=fake_mid, topic=fake_topic.encode())
#         message.payload = fake_payload
#         message.qos = fake_qos
#         return message

#     @pytest.mark.it(
#         "Triggers on_http_message_received_handler event handler upon receiving message"
#     )
#     def test_calls_event_handler_callback(self, mocker, mock_http_client, transport, message):
#         callback = mocker.MagicMock()
#         transport.on_http_message_received_handler = callback

#         # Manually trigger Paho on_message event_handler
#         mock_http_client.on_message(client=mock_http_client, userdata=None, http_message=message)

#         # Verify transport.on_http_message_received_handler was called
#         assert callback.call_count == 1
#         assert callback.call_args == mocker.call(message.topic, message.payload)

#     @pytest.mark.it(
#         "Skips on_http_message_received_handler event handler if set to 'None' upon receiving message"
#     )
#     def test_skips_none_event_handler_callback(self, mocker, mock_http_client, transport, message):
#         assert transport.on_http_message_received_handler is None
...
#     @pytest.mark.it("Recovers from Exception in on_http_message_received_handler event handler")
#     def test_event_handler_callback_raises_exception(
#         self, mocker, mock_http_client, transport, message, arbitrary_exception
#     ):
#         event_cb = mocker.MagicMock(side_effect=arbitrary_exception)
#         transport.on_http_message_received_handler = event_cb
...
#     @pytest.mark.it(
#         "Allows any BaseExceptions raised in on_http_message_received_handler event handler to propagate"
#     )
#     def test_event_handler_callback_raises_base_exception(
#         self, mocker, mock_http_client, transport, message, arbitrary_base_exception
#     ):
#         event_cb = mocker.MagicMock(side_effect=arbitrary_base_exception)
#         transport.on_http_message_received_handler = event_cb

#         with pytest.raises(arbitrary_base_exception.__class__) as e_info:
#             mock_http_client.on_message(
#                 client=mock_http_client, userdata=None, http_message=message
#             )
#         assert e_info.value is arbitrary_base_exception


# @pytest.mark.describe("HTTPTransport - Misc.")
# class TestMisc(object):
#     @pytest.mark.it(
#         "Handles multiple callbacks from multiple different types of operations that complete out of order"
#     )
#     def test_multiple_callbacks_multiple_ops(self, mocker, mock_http_client, transport):
#         callback1 = mocker.MagicMock()
#         callback2 = mocker.MagicMock()
#         callback3 = mocker.MagicMock()
...
#         mock_http_client.subscribe.return_value = (fake_rc, mid1)
#         mock_http_client.publish.return_value = http.MQTTMessageInfo(mid2)
#         mock_http_client.unsubscribe.return_value = (fake_rc, mid3)
