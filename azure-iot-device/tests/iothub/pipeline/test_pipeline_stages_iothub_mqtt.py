...
class IoTHubMQTTTranslationStageInstantiationTests(IoTHubMQTTTranslationStageTestConfig):
    @pytest.mark.it("Initializes 'feature_to_topic' attribute as an empty dictionary")
    def test_auth_provider(self, init_kwargs):
        stage = pipeline_stages_iothub_mqtt.IoTHubMQTTTranslationStage(**init_kwargs)
        assert stage.feature_to_topic == {}

    @pytest.mark.it("Initializes 'device_id' attribute as None")
    def test_device_id(self, init_kwargs):
        stage = pipeline_stages_iothub_mqtt.IoTHubMQTTTranslationStage(**init_kwargs)
        assert stage.device_id is None

    @pytest.mark.it("Initializes 'module_id' attribute as None")
...
pipeline_stage_test.add_base_pipeline_stage_tests(
    ...
    extended_stage_instantiation_test_class=IoTHubMQTTTranslationStageInstantiationTests,
)
...
class TestIoTHubMQTTTranslationStageRunOpWithSetIoTHubConnectionArgsOperation(
    StageRunOpTestBase, IoTHubMQTTTranslationStageTestConfig
):
    # CT-TODO: more fixturization
    ...
    @pytest.mark.it(
        "Sets the device_id and module_id from the operation in the corresponding attributes of the stage"
    )
    def test_cache_ids(self, stage, op):
        assert stage.device_id is None
        assert stage.module_id is None
    ...
    @pytest.mark.it(
        "Caches topic names formatted with the provided connection details in the 'feature_to_topic' attribute of the stage"
    )
    def test_cache_topics(self, stage, op):
        assert stage.feature_to_topic == {}
    ...
    @pytest.mark.it("Sends a new SetMQTTConnectionArgsOperation down the pipeline")
    def test_sends_new_op_down(self, mocker, stage, op):
        stage.run_op(op)
        ...
        if op.module_id:
            assert new_op.client_id == "{device_id}/{module_id}".format(
                device_id=op.device_id, module_id=op.module_id
            )
        else:
            assert new_op.client_id == op.device_id

        # HOSTNAME CONFIGURATION
        if op.gateway_hostname:
            assert new_op.hostname == op.gateway_hostname
        else:
            assert new_op.hostname == op.hostname
...
@pytest.mark.describe(
    "IoTHubMQTTTranslationStage - .run_op() -- Called with UpdateSasTokenOperation while already connected"
)
class TestIoTHubMQTTTranslationStageRunOpWithUpdateSasTokenWhileConnected(
    StageRunOpTestBase, IoTHubMQTTTranslationStageTestConfig
):
    @pytest.fixture
    def stage(self, mocker, cls_type, init_kwargs):
    ...
    @pytest.mark.it("Allows completion of the operation, if it is completed with error")
    def test_complete_with_error(self, mocker, stage, op, arbitrary_exception):
        stage.run_op(op)
        assert stage.send_op_down.call_count == 1
        ...
        # No further ops sent down
        assert stage.send_op_down.call_count == 1

    @pytest.mark.it(
        "Halts the UpdateSasTokenOperation's completion and sends a new ReauthorizeConnectionOperation down the pipeline if the UpdateSasTokenOperation is completed successfully"
    )
    def test_send_reauthorize_after_completion(self, mocker, stage, op):
        stage.run_op(op)
        assert stage.send_op_down.call_count == 1
        ...
        new_op = stage.send_op_down.call_args[0][0]
        assert isinstance(new_op, pipeline_ops_base.ReauthorizeConnectionOperation)

    @pytest.mark.it(
        "Completes the original UpdateSasTokenOperation successfully if the ReauthorizeConnectionOperation completes successfully"
    )
    def test_reauthorize_connection_completed_success(self, mocker, stage, op):
        stage.run_op(op)
        assert stage.send_op_down.call_count == 1
        op.complete()
        assert not op.completed
    ...
    @pytest.mark.it(
        "Completes the original UpdateSasTokenOperation with the error from the ReauthorizeConnectionOperation, if the ReauthorizeConnectionOperation completes with error"
    )
    def test_reauthorize_connection_completed_failure(self, mocker, stage, op, arbitrary_exception):
        stage.run_op(op)
        assert stage.send_op_down.call_count == 1
        op.complete()
        assert not op.completed
...
@pytest.mark.describe(
    "IoTHubMQTTTranslationStage - .run_op() -- Called with UpdateSasTokenOperation while not already connected"
)
class TestIoTHubMQTTTranslationStageRunOpWithUpdateSasTokenWhileNotConnected(
    StageRunOpTestBase, IoTHubMQTTTranslationStageTestConfig
):
    @pytest.fixture
    def stage(self, mocker, cls_type, init_kwargs):
    ...
    @pytest.mark.it("Does not alter the operation's later completion")
    @pytest.mark.parametrize(
        "error", [pytest.param(True, id="With error"), pytest.param(False, id="Successful")]
    )
    def test_complete_later(self, mocker, stage, op, arbitrary_exception, error):
        stage.run_op(op)
        assert not op.completed
        if error:
            op.complete(error=arbitrary_exception)
...
class TestIoTHubMQTTTranslationStageRunOpWithSendD2CMessageOperation(
    ...
    # @pytest.fixture(params=["utf-8"])
    # def content_encoding(self):

    # @pytest.fixture
    # def message(self, content_type):
    #     return Message(data="fake_message")

    @pytest.fixture
    def op(self, mocker, message):
        return pipeline_ops_iothub.SendD2CMessageOperation(
            message=Message(data="fake_message"), callback=mocker.MagicMock()
        )

    @pytest.mark.it("Sends a new MQTTPublishOperation down the pipeline")
    def test_sends_publish_down(self, mocker, op, stage):
        stage.run_op(op)
    ...
    # @pytest.mark.it("Encodes the message from the SendD2CMessageOperation in the MQTT topic of the MQTTPublishOperation")
