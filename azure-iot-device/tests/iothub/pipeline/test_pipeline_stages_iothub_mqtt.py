...
class TestIoTHubMQTTTranslationStageHandlePipelineEventWithIncomingMQTTMessageEventInputTopic(
    ...
    @pytest.mark.it(
        "Extracts the input name from the topic, and attaches it to the newly created Message"
    )
    def test_input_name_on_message(self, event, stage, mock_mqtt_topic, input_name):
        stage.handle_pipeline_event(event)

        # Get Message that was created
        assert mock_mqtt_topic.extract_message_properties_from_topic.call_count == 1
        message = mock_mqtt_topic.extract_message_properties_from_topic.call_args[0][1]
        # The Message contains the input
        assert message.input_name == input_name
