...
from . import (
    constant,
    pipeline_stages_storageblob,
    pipeline_events_storageblob,
    pipeline_ops_storageblob,
...
class StorageBlobPipeline(object):
    def __init__(self, auth_provider, pipeline_configuration):
        ...
        self._pipeline = (
            ...
            .append_stage(pipeline_stages_storageblob.UseAuthProviderStage())
            .append_stage(pipeline_stages_iothub_http.StorageBlobHTTPTranslationStage())
        ...
        def _on_pipeline_event(event):
            if isinstance(event, pipeline_events_storageblob.C2DMessageEvent):
            ...
            elif isinstance(event, pipeline_events_storageblob.InputMessageEvent):
            ...
            elif isinstance(event, pipeline_events_storageblob.MethodRequestEvent):
            ...
            elif isinstance(event, pipeline_events_storageblob.TwinDesiredPropertiesPatchEvent):
        ...
        if isinstance(auth_provider, X509AuthenticationProvider):
            op = pipeline_ops_storageblob.SetX509AuthProviderOperation(
                auth_provider=auth_provider, callback=callback
            )
        else:  # Currently everything else goes via this block.
            op = pipeline_ops_storageblob.SetAuthProviderOperation(
                auth_provider=auth_provider, callback=callback
            )
    ...
    def connect(self, callback):
        ...
        :raises: :class:`azure.iot.device.storageblob.pipeline.exceptions.ConnectionFailedError`
        :raises: :class:`azure.iot.device.storageblob.pipeline.exceptions.ConnectionDroppedError`
        :raises: :class:`azure.iot.device.storageblob.pipeline.exceptions.UnauthorizedError`
        :raises: :class:`azure.iot.device.storageblob.pipeline.exceptions.ProtocolClientError`
        """
    ...
    def send_message(self, message, callback):
        ...
        self._pipeline.run_op(
            pipeline_ops_storageblob.SendD2CMessageOperation(message=message, callback=on_complete)
        )
    ...
    def send_output_event(self, message, callback):
        ...
        self._pipeline.run_op(
            pipeline_ops_storageblob.SendOutputEventOperation(message=message, callback=on_complete)
        )
    ...
    def send_method_response(self, method_response, callback):
        ...
        :raises: :class:`azure.iot.device.storageblob.pipeline.exceptions.ProtocolClientError`
        """
        logger.debug("StorageBlobPipeline send_method_response called")

        def on_complete(op, error):
            callback(error=error)

        self._pipeline.run_op(
            pipeline_ops_storageblob.SendMethodResponseOperation(
    ...
    def get_twin(self, callback):
        ...
        self._pipeline.run_op(pipeline_ops_storageblob.GetTwinOperation(callback=on_complete))
    ...
    def patch_twin_reported_properties(self, patch, callback):
        ...
        self._pipeline.run_op(
            pipeline_ops_storageblob.PatchTwinReportedPropertiesOperation(
    ...
    def get_storage_info(self, callback):
        ...
        self._pipeline.run_op(
            pipeline_ops_storageblob.GetStorageInfoOperation(callback=on_complete)
        )
