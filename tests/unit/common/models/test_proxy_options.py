...
class TestProxyOptions(object):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                socks.SOCKS4, "SOCKS4", socks.SOCKS4, id="SOCKS4 (socks constant)"
            ),
            pytest.param(
                socks.SOCKS5, "SOCKS5", socks.SOCKS5, id="SOCKS5 (socks constant)"
            ),
        ],
    )
    def test_proxy_type(
        self, proxy_type_input, expected_proxy_type, expected_proxy_type_socks
    ):
    ...
    def test_proxy_address(self):
        ...
        proxy_options = ProxyOptions(
            proxy_type=socks.HTTP, proxy_addr=proxy_addr, proxy_port=8888
        )
    ...
    def test_proxy_address_read_only(self):
        proxy_options = ProxyOptions(
            proxy_type=socks.HTTP, proxy_addr="127.0.0.1", proxy_port=8888
        )
        with pytest.raises(AttributeError):
