...
class TestSendMessage(object):
    ...
    async def test_send_message_simple(self, leak_tracker, session, random_message, service_helper):
    ...
    async def test_bad_payload_raises(self, leak_tracker, session):
        leak_tracker.set_initial_object_list()
        ...
        async with session:
            ...
            with pytest.raises(asyncio.CancelledError) as e_info:
                await session.send_message(thing_that_cant_serialize)
    ...
    async def test_sends_json_string(self, leak_tracker, session, service_helper):
        leak_tracker.set_initial_object_list()
    ...
    async def test_sends_random_string(self, leak_tracker, session, service_helper):
        leak_tracker.set_initial_object_list()
