...
import pytest
from provisioningserviceclient.utils.auth import (
...
def test_create_with_parsed_values(hostname, keyname, key):
    """Test creation from already parsed values
    """
    new_cs = ConnectionStringAuthentication.create_with_parsed_values(hostname, keyname, key)
    assert type(new_cs) == ConnectionStringAuthentication
    assert new_cs[HOST_NAME] == hostname
    assert new_cs[SHARED_ACCESS_KEY_NAME] == keyname
    assert new_cs[SHARED_ACCESS_KEY] == key
...
def test_signed_session(mocker, valid_cs_auth, hostname, keyname, key):
    ...
    """
    mock_sas = mocker.patch("provisioningserviceclient.utils.auth.SasToken", autospec=True)
