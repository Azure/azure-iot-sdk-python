...
MSG_TXT = "{\"deviceId\": \"myPythonDevice\",\"windSpeed\": %.2f,\"temperature\": %.2f,\"humidity\": %.2f,\"macAddress\":%s,\"brand\":%s,\"hardware\":%s,\"revisionNumber\":%s,\"serialNumber\":%s}"
...
def iothub_client_sample_run():
    ...
    try:
		...
		macaddress=bytearray(display_all_macaddress(), 'utf8')
                brand=get_cpu_info("model name")	
		hardwarenumber=get_cpu_info("hardware")
		revisionnumber=get_cpu_info("revision")
		serialnumber=get_cpu_info("serial")
                msg_txt_formatted = MSG_TXT % (
                    AVG_WIND_SPEED + (random.random() * 4 + 2),
                    temperature,
                    humidity,macaddress,brand,hardwarenumber,revisionnumber,serialnumber)
...
def vendor_details():
	    ...
	    mac=get_mac_address(ifname=str(iface_name))
            print("vendor details for [%s] as bellow:" %iface_name)
            print("%s" % requests.get("http://macvendors.co/api/"+mac).text)
...
if __name__ == '__main__':
    ...
    print("All MAC Address: %s" % display_all_macaddress())
    vendor_details()
